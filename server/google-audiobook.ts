import { generateChapterAudio, type GoogleTTSOptions } from './google-tts';
...
export async function generateFullAudiobook(
  ...
  try {
    for (let i = 0; i < chapters.length; i++) {
      ...
      console.log(`Generating Google TTS audio for chapter ${i + 1}/${chapters.length}: ${chapter.title}`);
      ...
      // Generate audio for this chapter using Google TTS
    ...
    console.log(`Google TTS audiobook generation complete! Generated ${chapterFiles.length} chapters, total duration: ${totalDuration}s`);
    ...
    console.error('Error generating Google TTS audiobook:', error);
