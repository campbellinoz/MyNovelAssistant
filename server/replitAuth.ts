...
async function upsertUser(
  ...
  console.log(`Creating/updating user: ${claims["email"]} (ID: ${claims["sub"]})`);
...
export async function setupAuth(app: Express) {
  ...
  // Get all domains including development
  const domains = process.env.REPLIT_DOMAINS ? 
    process.env.REPLIT_DOMAINS.split(",").map(d => d.trim()) : 
    [];
  
  // Always add current hostname for development
  if (process.env.REPL_ID) {
    const devDomain = `${process.env.REPL_ID}-00-1e7mkzesg3xe5.picard.replit.dev`;
  ...
  app.get("/api/callback", (req, res, next) => {
    console.log(`Auth callback for domain: ${req.hostname}`);
    passport.authenticate(`replitauth:${req.hostname}`, {
